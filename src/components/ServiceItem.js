      ...
      <TouchableOpacity onPress={() => onPress(item)}>
        <View style={styles.topLine}>
          ...
          <TouchableOpacity onPress={() => onBookmarkServiceItem(item, Constants.user?.favorsids?.includes(item.id) ? 'delete' : 'add')}>
            ...
            <Image resizeMode="contain" style={{ height: 22, width: 22, tintColor:Constants.user?.favorsids?.includes(item.id) ? Colors.gold : Colors.greyColor}}
                                source={Constants.user?.favorsids?.includes(item.id) ? Images.ic_favorite_active:Images.ic_favorite_inactive} />
          </TouchableOpacity>
        </View>
